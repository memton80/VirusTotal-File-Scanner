...
async function updateUI() {
  ...
  if (enabled) {
    ...
    toggleBtn.appendChild(VTIcons.pause(15));
    toggleBtn.className = 'btn btn-icon';
    toggleBtn.title = VTUtils.t('btnPause');
    header.classList.remove('paused');
    statusBanner.classList.remove('show');
  } else {
    toggleBtn.textContent = '';
    toggleBtn.appendChild(VTIcons.play(15));
    toggleBtn.className = 'btn btn-icon state-paused';
    toggleBtn.title = VTUtils.t('btnResume');
  ...
  try {
    ...
    for (const r of history) {
      ...
      if (r.skipped && r.reason === 'sensitive') {
        ...
        fileIcon.style.stroke = 'var(--br-warning)';
