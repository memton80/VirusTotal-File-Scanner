...
async function uploadBlobToVT(blob, apiKey, itemMeta = {}) {
  ...
  try {
    ...
    await startPolling(analysisId, apiKey, itemMeta);
...
// === POLLING BASÉ SUR LES ALARMES (MV3 compatible) ===
// Remplace le polling setTimeout qui était tué par Firefox entre les sleeps.
// L'état est persisté dans storage → survit au redémarrage du background script.

const POLL_KEY_PREFIX = 'vt_poll_';
...
async function startPolling(analysisId, apiKey, itemMeta) {
  if (!analysisId || typeof analysisId !== 'string' || !ANALYSIS_ID_REGEX.test(analysisId)) {
    console.error("ID d'analyse invalide ou format inattendu:", typeof analysisId);
    VTUtils.notify(
  ...
  await browser.storage.local.set({
    [key]: {
      analysisId,
      itemMeta: {
        filename: itemMeta.filename || 'unknown',
        id: itemMeta.id || null,
        sha256: itemMeta.sha256 || null
      },
      attempts: 0,
      consecutiveErrors: 0,
  ...
  browser.alarms.create(key, { delayInMinutes: POLL_INITIAL_DELAY_SECONDS / 60 });
  console.log(`[Polling] Alarme créée pour: ${analysisId}`);
}

browser.alarms.onAlarm.addListener(async (alarm) => {
  if (!alarm.name.startsWith(POLL_KEY_PREFIX)) return;
  ...
  const { analysisId, itemMeta, attempts, consecutiveErrors, delaySeconds } = state;

  // Clé API lue fraîchement — jamais stockée dans le state de polling
  const apiKey = await VTUtils.getApiKey();
  if (!apiKey) {
    await browser.storage.local.remove(key);
    VTUtils.notify(
      `vt-no-api-poll-${analysisId}`,
      VTUtils.t('notifNoApiKey'),
      VTUtils.t('notifNoApiKeyMsg')
    );
    return;
  ...
  const safeFilename = VTUtils.sanitizeFilename(VTUtils.extractFilename(itemMeta.filename));

  console.log(`[Polling] Tentative ${attempts + 1}/${POLL_MAX_ATTEMPTS} pour: ${analysisId}`);

  if (attempts >= POLL_MAX_ATTEMPTS) {
    await browser.storage.local.remove(key);
    VTUtils.notify(
      `vt-timeout-${analysisId}`,
  ...
  try {
    ...
    const r = await fetch(`${VT_ANALYSIS_ENDPOINT}/${encodeURIComponent(analysisId)}`, {
      headers: { 'x-apikey': apiKey },
      signal: controller.signal
    });
    clearTimeout(timeoutId);
    ...
    if (r.status === 429) {
      console.warn('[Polling] Rate limit, attente 2 min');
      await browser.storage.local.set({
        [key]: { ...state, attempts: attempts + 1, consecutiveErrors: 0, delaySeconds: 120 }
      });
      browser.alarms.create(key, { delayInMinutes: 2 });
      return;
    }
    ...
    if (r.ok) {
      ...
      if (status === 'completed') {
        await browser.storage.local.remove(key);
        console.log('[Polling] Scan terminé!');

        const sha256FromId = j?.data?.id?.match(/u-([a-f0-9]{64})-/i)?.[1] || null;
      ...
      // Scan encore en cours — replanifier
      const newDelay = Math.min(delaySeconds * 1.5, POLL_MAX_DELAY_SECONDS);
      await browser.storage.local.set({
        [key]: { ...state, attempts: attempts + 1, consecutiveErrors: 0, delaySeconds: newDelay }
      });
      browser.alarms.create(key, { delayInMinutes: newDelay / 60 });

    } else {
      // Erreur HTTP non fatale
      const newConsecutiveErrors = consecutiveErrors + 1;
      if (newConsecutiveErrors >= 3) {
        await browser.storage.local.remove(key);
        VTUtils.notify(
          `vt-polling-failed-${analysisId}`,
      ...
      const newDelay = Math.min(delaySeconds * 1.5, POLL_MAX_DELAY_SECONDS);
      await browser.storage.local.set({
        [key]: { ...state, attempts: attempts + 1, consecutiveErrors: newConsecutiveErrors, delaySeconds: newDelay }
      });
      browser.alarms.create(key, { delayInMinutes: newDelay / 60 });
    }
...
(async () => {
  ...
  for (const [key, val] of Object.entries(all)) {
    if (key.startsWith(POLL_KEY_PREFIX) && val?.analysisId) {
      console.log(`[Init] Reprise du polling interrompu: ${val.analysisId}`);
      browser.alarms.create(key, { delayInMinutes: 0.1 });
    }
  }
