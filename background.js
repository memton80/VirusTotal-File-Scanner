// === VirusTotal Auto Scanner - OPTIMISÉ avec cache et lookup SHA256 ===
...
console.log("🚀 VirusTotal Scanner loaded (v1.9.0 - OPTIMIZED)");
...
async function calculateSHA256(blob) {
  try {
    ...
    console.log(`✅ SHA-256 calculé: ${hashHex}`);
    return hashHex;
  } catch (e) {
...
const scanCache = {
  async get(sha256) {
    try {
      const result = await browser.storage.local.get(`cache_${sha256}`);
      const cached = result[`cache_${sha256}`];
      ...
      if (cached && cached.timestamp) {
        ...
        if (age < maxAge) {
          console.log(`💾 Résultat en cache trouvé (${Math.floor(age / 1000 / 60 / 60)}h)`);
          return cached;
        } else {
  ...
  async set(sha256, data) {
    try {
      ...
      await browser.storage.local.set({ [`cache_${sha256}`]: cacheData });
  ...
  async remove(sha256) {
    try {
      await browser.storage.local.remove(`cache_${sha256}`);
    } catch (e) {
...
async function lookupFileBySHA256(sha256, apiKey) {
  ...
  try {
    ...
    if (response.status === 429) {
      console.warn("⚠️ Rate limit atteint lors du lookup");
      return null;
    }
    
    if (!response.ok) {
      console.warn(`⚠️ Lookup failed: ${response.status}`);
      return null;
    }
    ...
    if (e.name === 'AbortError') {
      console.warn("⏱️ Timeout du lookup");
    } else {
  ...
  try {
    ...
    if (url) {
      try {
        ...
        // === OPTIMISATION: Calcul SHA-256 et vérification cache ===
        ...
        if (sha256) {
          ...
          if (cached) {
            ...
            notify(
              `vt-cached-${sha256}`,
              `${title} (💾 Cache)`,
              `${safeFilename}\n${cached.detections}/${cached.totalEngines} ${t('detections')} (${detectionRate.toFixed(1)}%)`
            );
...
async function fetchFileReportBySha(sha256, apiKey, itemMeta) {
  ...
  while (retries < maxRetries) {
    try {
      ...
      // Sauvegarder dans l'historique ET le cache
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { uploadBlobToVT, calculateSHA256, lookupFileBySHA256 };
}